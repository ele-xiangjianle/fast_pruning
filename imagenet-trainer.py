...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            if args.finetune > 0:
                model = torch.load(args.resume, map_location={
                       'cuda:%d' % 0: 'cuda:%d' % args.gpu})
                model = model.cuda(args.gpu)
                optimizer = torch.optim.SGD(model.parameters(), lr=args.lr,
                          momentum=args.momentum, weight_decay=args.weight_decay)
                if os.path.exists('result/best_checkpoint.pth.tar'):
                    best_acc1 = load_best_acc('result/best_checkpoint.pth.tar')
                print("Load Success!")
            else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.finetune > 0:
            is_global_best = best_acc1 > global_best_acc1
            global_best_acc1 = max(global_best_acc1, best_acc1)
...
def save_checkpoint(args, state, is_best, is_global_best=False, filename=os.path.join('result', 'checkpoint.pth.tar')):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename,  os.path.join('result', f'{args.filename}.pth.tar'))
    if is_global_best:
        shutil.copyfile(filename,  os.path.join('result', 'best_checkpoint.pth.tar'))
